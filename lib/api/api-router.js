...
module.exports = class ApiRouter {
    ...
    route (app, routes) {
        ...
        routes.forEach(route => {
            ...
            if (preRoutingMiddlewares && Array.isArray(preRoutingMiddlewares)) {
                preRoutingMiddlewares.forEach(middleware => {
                    try {
                        apiRouter.use(middleware());
                    } catch (e) {
                        global.logger.warn('Prerouting middleware not found, skipping');
                    }
                });
            }
